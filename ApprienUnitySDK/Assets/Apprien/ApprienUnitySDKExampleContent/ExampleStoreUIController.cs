...
namespace ApprienUnitySDK.ExampleProject
{
    public class ExampleStoreUIController : MonoBehaviour, IStoreListener
    {
        ...
        [SerializeField]
        private ApprienConnection _apprienConnection;
        ...
        [SerializeField]
        private string _IAPCatalogResource;

        [SerializeField]
        private string _subscriptionCatalogResource;
        ...
        void Awake()
        {
            if (_apprienConnection == null || _apprienConnection.Token.Length == 0)
            {
        ...
        private void InitializeProducts()
        {
            ...
            var catalogFile = Resources.Load<TextAsset>(_currentType == CanvasType.IAPs ? _IAPCatalogResource : _subscriptionCatalogResource);
        ...
        public void SwitchButtonPressed()
        {
            ...
            _IAPCanvas.SetActive(false);
            _subscriptionCanvas.SetActive(false);
            if (_currentType == CanvasType.IAPs)
            {
                _currentType = CanvasType.Subscriptions;
                _subscriptionCanvas.SetActive(true);
            }
            else
            {
                _currentType = CanvasType.IAPs;
                _IAPCanvas.SetActive(true);
            }
